...
import { useRegister, useVerifyUser } from '../hooks/auth';
import { useOtp, useVerifyOtp } from '../hooks/otp';
...
function Register() {
  ...
  const {
    ...
    formState: { errors },
    watch,
  } = useForm({ resolver: yupResolver(scheme), defaultValues });

  const { mutate: register } = useRegister();
  const { mutate: verifyUser, isSuccess: userExisted } = useVerifyUser();
  const { mutate: getOtp, isSuccess: isSuccesOtp } = useOtp();
  const { mutate: verifyOtp, isSuccess: verfiedOtp } = useVerifyOtp();
  ...
  useEffect(() => {
    verfiedOtp &&
      register({
        fullname: `${watch('firstName')} ${watch('lastName')}`,
        email: watch('email'),
        password: watch('confirm'),
        birthday: '1-1-1970',
      });
  }, [verfiedOtp, register, watch, handleSubmit]);
  useEffect(() => {
      ...
      <div className="mx-auto flex h-screen flex-col items-center justify-center gap-8">
        ...
        >
          ...
          <p className="pb-6 text-center text-sm leading-6 text-gray-700">
            Thông tin đăng nhập chỉ được sử dụng để xác thực trong {APP_NAME}.
            Tất cả dữ liệu đã lưu sẽ được lưu trữ trong cơ sở dữ liệu của bạn.
          </p>
          ...
          <div className="mb-6">
            ...
            >
              ...
              <input
                type="text"
                id="email"
                className={`mt-2 block w-full rounded-lg border border-gray-300 bg-gray-50 p-4 text-gray-900 focus:outline-blue-600 ${
                  errors.email &&
                  'border-2 border-red-600 focus:outline-red-600'
                }`}
                {...registerInput('email')}
            ...
            {userExisted && (
              <p className="text-sm text-red-400">Tài khoản đã tồn tại</p>
            )}
          </div>
          <div className="mb-6">
            <label
            ...
            <div className="mb-6">
              ...
              >
                ...
                <input
                  ...
                  {...registerInput('otp', { required: true })}
                />
              </label>
            </div>
          )}
          <button
            type="submit"
            className="mr-2 w-full rounded-lg bg-blue-600 px-5 py-4 font-medium text-white transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-blue-300 disabled:bg-slate-400 dark:bg-gray-900 dark:focus:ring-blue-400"
          >
            {isSuccesOtp ? 'Xác nhận' : 'Đăng ký'}
          </button>

          <p className="mt-6">
            <span>Đã có tài khoản?</span>
            <Link to="/auth/login">
              <strong className="hover:underline"> Đăng nhập ngay</strong>
            </Link>
          </p>
        </form>
